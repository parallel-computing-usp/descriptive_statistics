...
int main(int argc,char **argv){
    ...
    {
        // região que permite a entrada de uma thread por vez
        #pragma omp single
		{
            ///////////////////////////////////////////////////////
            // Tasks sem dependencias
            ///////////////////////////////////////////////////////
            // task que calcula a média harmonica 
            #pragma omp task
			{
                calcula_media_harmonica(matriz, media_har,lin,col);
            }
            // task para cálculo da moda, 
            #pragma omp task
			{
            ...
            ///////////////////////////////////////////////////////
            // task que orderna as colunas e cria a depencia de saída utilizada no cálculo da mediana
            #pragma omp task depend(out:matriz)
			{
                ordena_colunas(matriz,lin,col);
            }
            // task para cálculo da mediana com dependencia de entrada oriunda da tasks de ordenação
            #pragma omp task depend(in:matriz)
			{
                calcula_mediana(matriz,mediana,lin,col);
            }
            
            // task que calcula a média e sinaliza uma dependencia de saida
            #pragma omp task depend(out:media)
			{
                calcula_media(matriz, media, lin, col);
            }
            // task para cálculo da variancia, sinaliza as dependencias de entrada e saida
            #pragma omp task depend(in: media) depend(out: variancia)
			{
                calcula_variancia(matriz, media, variancia, lin, col);
            }
            // task para cálculo da variancia, sinaliza as dependencia de entrada com a variancia
            #pragma omp task depend(in: variancia) depend(out: dp)
			{
                calcula_desvio_padrao(variancia, dp, col);
            }
            // task para cálculo da variancia, sinaliza as dependencia de entrada com a variancia
            #pragma omp task depend(in:media, dp)
			{
                calcula_coeficiente_variacao(media, dp, cv, col);
            }

            // região que aguarda todas as tasks e imprime os resultados sequencialmente
            #pragma omp taskwait 
            {
                for(i=0;i<col;i++)
                    printf("%.1lf ",media[i]);
